...
import { MessageUpdateType, type MessageUpdate } from "../../types/MessageUpdate";
...
export async function* generateTitleForConversation(
	...
	try {
		const userMessage = conv.messages.find((m: { from: string }) => m.from === "user");
		// HACK: detect if the conversation is new
		if (conv.title !== "New Chat" || !userMessage) return;

		const prompt = userMessage.content;
		const title = (await generateTitle(prompt, conv._id.toString())) ?? "New Chat";
		...
		logger.error(Error("Failed while generating title for conversation", { cause }));
	}
}

export async function generateTitle(prompt: string, conversationId: string): Promise<string | null> {
	...
	return await generateFromDefaultEndpoint({
		...
		contextId: `title_${conversationId}`,
	})
		.then((summary: string) => {
